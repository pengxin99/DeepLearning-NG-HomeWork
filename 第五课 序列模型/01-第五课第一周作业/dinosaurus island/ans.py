...
def sample(parameters, char_to_ix, seed):
    ...
    x = np.zeros((vocab_size, 1))                          # one-hot 编码，向量长度为单词长度
    # Step 1': Initialize a_prev as zeros (≈1 line)
    a_prev = np.zeros((n_a, 1))                            # 设置a变量
    ...
    while (idx != newline_character and counter != 50):
        ...
        a = np.tanh(np.dot(Waa, a_prev) + np.dot(Wax, x) + b)       # 计算每个cell前向传输的输出值
        z = np.dot(Wya, a) + by
        y = softmax(z)
        
        # for grading purposes
        np.random.seed(counter + seed)                              # 以seed随机机制，每次生成索引
        # Step 3: Sample the index of a character within the vocabulary from the probability distribution y
        idx = np.random.choice(len(char_to_ix), p=y.ravel())        # 找到一个以y为概率分布的单词索引号
        
        # Append the index to "indices"
        indices.append(idx)
        
        # Step 4: Overwrite the input character as the one corresponding to the sampled index.
        x = np.zeros((vocab_size,1))                                # 初始化当前的ont-hot编码
        x[idx] = 1                                                  # 进行 one-hot 编码
...
if __name__ == '__main__':
    ...
    # parameters = model(data, ix_to_char, char_to_ix, num_iterations=100000)

...
def yolo_filter_boxes(box_confidence, boxes, box_class_probs, threshold=.6):
    ...
    box_scores = box_confidence * box_class_probs       # 每个cell里面各种分类的概率
    ### END CODE HERE ###
    
    # Step 2: Find the box_classes thanks to the max box_scores, keep track of the corresponding score
    ### START CODE HERE ### (≈ 2 lines)
    box_classes = K.argmax(box_scores, axis=-1)         # 各种分类概率里面找到值最大的
    box_class_scores = K.max(box_scores, axis=-1, keepdims=False)       # 值最大的分类的概率，即score
    ### END CODE HERE ###
    
    # Step 3: Create a filtering mask based on "box_class_scores" by using "threshold". The mask should have the
    # same dimension as box_class_scores, and be True for the boxes you want to keep (with probability >= threshold)
    ### START CODE HERE ### (≈ 1 line)
    filtering_mask = box_class_scores >= threshold      # mask，用与屏蔽threshold值以下的cell
    ### END CODE HERE ###
